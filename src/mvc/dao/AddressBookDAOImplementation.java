...
public class AddressBookDAOImplementation implements AddressBookDAO{
    ...
    {   
       try
       {
          conn = DBConnection.getInstance().getConnect();
           ...
           String qry;
           qry = "insert into AddressBook values ('"+person.getName()+"','"+person.getMob()+"','"+person.getEmail()+"','"+person.getQualification()+"')";
   ...
   @Override
    public void searchPerson(String name)
    {
        try
        {
            conn=conn = DBConnection.getInstance().getConnect();
           st = conn.createStatement();
           String qry="search from AddressBook where name like '"+name+"'";
          st.executeUpdate(qry);
           conn.close();
        }
       catch (SQLException e)
    ...
    @Override
     public void updatePerson(Person person,String name)
     {
         try
       {
           conn = DBConnection.getInstance().getConnect();
           st = conn.createStatement();
            String qry="update AddressBook set name='"+person.getName()+"',mob='"+person.getMob()+"',email='"+person.getEmail()+"' where name='"+name+"',Qualification='"+person.getQualification();
           st.executeUpdate(qry);
           conn.close();
        }
    ...
    public void getSelectedName(DetailViewPanel detailPanel,String selectedName)
    {
        try {
            ...
            while(rs.next()){
             ...
             detailPanel.setQualification(rs.getString("Qualification"));
